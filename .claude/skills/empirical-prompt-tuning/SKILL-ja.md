...
5. **差分適用**: 不明瞭点を潰す最小修正をプロンプトに入れる。1 イテレーション 1 テーマ（関連する複数修正は OK、無関係な修正は次回に回す）。
   ...
   - **不明瞭点が「事実の欠落・誤り」型のときは、何かを書く前に帰属判定を行う**（後述「事実誤りの帰属判定」）: 一次ソースで裏取りし、プラットフォーム側の欠陥（報告する。回避策を書かない）か指示側の欠陥（断定形で書く）かを決める。
...
## 事実誤りの帰属判定（プラットフォーム側 vs 指示側）

不明瞭点が **事実の欠落・誤り**（自分の書き方の曖昧さではなく）だった場合、反射的にプロンプトを直さない。

**まず一次ソースで事実を裏取りする**（公式ドキュメント・実装・live な API / MCP サーバー）。**実行者のレポートを一次ソース扱いにしない**——実行者は自分と同じ上流テキストを読んでいるので、複数体が同じ誤読を共有しうる。全員が一致していても誤りであることがある。

**次に、どちら側の欠陥かを判定する。** フィードバック経路があるプラットフォーム（Kuroco なら `kuroco_feedback-send`）は、**分かりにくい仕様・薄いツール `description`・記載のないリファレンス**を直せる。その修正は恒久的だが、回避策を指示側に書くと、プラットフォームが改善された瞬間に指示側が誤情報源になる。

**判定基準は「プラットフォーム側が直ったら、この記述は誤りになるか」。** YES → プラットフォーム側の欠陥。報告し、回避策は書かない。NO → 指示側の欠陥。断定形で事実を書く。

| 症状 | 帰属 | やること |
|---|---|---|
| 実装バグ（宣言済みフィールドが保存されない／文書化されたパラメータが無視される） | プラットフォーム | 報告のみ。**回避策を書かない**——直った時点で指示側が誤りになる |
| ツールの `description` / API リファレンス / 管理画面のラベルが曖昧・不足で、**独立した実行者2体以上が同じ誤読をした** | **両方** | 文言の修正を報告（現在の文言を引用し、置き換え案を添える）**と同時に**、指示側には正しい事実を断定形で書く。仕様の説明は回避策ではないので、文言が改善されても誤りにならない |
| 命名の非対称（同じ概念がオペレーションごとに別のパラメータ名で露出している） | **両方** | 同上。分かりにくさを報告し、指示側には正しい名前と使い分けを書く |
| 公式ドキュメントに記載のない挙動 | **両方** | 記載の追加を依頼する。指示側では**その1点だけ**を「実機で確認する」と限定する（周辺全体を未確認扱いにしない） |
| 機能ギャップ（欲しいオペレーションがそもそも存在しない） | プラットフォーム（要望）＋指示側 | **現行仕様での組み方**を書き、「現行仕様では X 操作は無い」と現行仕様であることを明示し、報告候補に挙げる。バグとは区別する——実行者は今日どうするかを知る必要がある |
| 自分のスキルの誤記・古い記述・写し間違い | 指示側のみ | プロンプトを直す。報告は不要 |

**独立した実行者2体が同じ誤読をしたら、それは自分のプロンプトだけでなく上流テキストについての証拠である。** 両者は同じ上流テキストを読んで同じように間違えた——その文言が修正候補。台帳に記録する。

**報告は外部送信なので、先に承認を得る。** 送信されるフィールドをユーザーに提示し、明示的な承認を得てからフィードバックツールを呼ぶ。承認が得られない、または経路がない場合は、**報告候補を成果物にリストとして残す**（症状／問題のある文言そのまま／置き換え案／誰が報告するか）。発見を失わないため。

### 暫定の記述と、恒久的な指示を分ける

**上流が今は分かりにくい・足りないことが理由で書いている記述**は、恒久的な指示とは種類が違い、後で消せるようにしておくべきもの。上の表の「両方」の行がそれを生む——曖昧な description や機能ギャップがあると、今日の回り道を書かざるを得ず、その記述には賞味期限がある。2種類を物理的に区別できる形にする:

- **事実そのものは使う場所に残す。** 実行者が必要とする事実を別の「メモ」ファイルに移すと、実行者はそれを読まずに誤った成果物を出す。分離するのは*メタ情報*——なぜその記述があるか、いつ消せるか。
- 暫定の記述にはその場に grep 可能な印を置く（HTMLコメント `<!-- spec-note: SN-3 -->` が使いやすい: レンダリング時に見えず、grep 一回で全箇所を洗える）。そして**保守者向け**のファイル1枚に、記述の在り処／なぜ書いたか／削除・縮小の条件／報告状況（受付番号）を一覧する。
- そのファイルが保守者向けであり、**作業中には読まなくてよい**ことを指示側に明記する。実行者の読む量を増やさないため。
- **記述と台帳エントリは同時に消す。** 片方だけ消すと、指示側に古い注記が残るか、追跡されない記述が残る。
- **載せないもの**: 安定した仕様の説明（モデルのレスポンス形、評価順序、文書化されたパラメータ、設定項目の意味）。上流が改善されても誤りにならないので削除条件を持たず、印を付けずに指示側に置く。
