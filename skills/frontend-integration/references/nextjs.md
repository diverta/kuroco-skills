...
> **Server Component の fetch にブラウザのCookieは乗らない。** 以下の一覧・詳細の例は公開コンテンツ（認証なしAPI）を前提にしている。Cookie認証が必要な画面はクライアントコンポーネント（`'use client'`）でブラウザから直接fetchすること（`credentials: 'include'` が効くのはブラウザ発のリクエストのみ）。
