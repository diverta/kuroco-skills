...
interface NewsDetail {
  ...
  contents: string
  ymd: string
  // 拡張項目はコンテンツ定義で ext_slug を設定し、スラッグ名で受けるのが推奨。
  // 未設定だとキー名が位置依存になり、形式もサイト単位で分かれる
  // （`ext_1` 形式 / `ext_col_01` 形式。Admin MCP の whoami の
  //  site.topics_ext_key_format で確認できる）。項目を並べ替えると壊れる。
  lead_text?: string
}
